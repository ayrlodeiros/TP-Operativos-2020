...
int crear_conexion_del_cliente(char *ip, char* puerto, t_log* mi_log) {
	...
	log_info(mi_log, "Un proceso se ha conectado exitosamente al broker en la IP: %s y PUERTO: %s", ip, puerto);
...
void levantar_servidor(char* ip, int port) {
	...
	if(socket_servidor == -1) {
		...
		log_info(mi_log, "Servidor levantado en IP: %s y PUERTO: %s", ip, puerto);
...
void esperar_cliente(int socket_servidor)
{
	...
	if(socket_cliente == -1) {
		...
		if( err != 0){
			log_info(mi_log,"Hubo un error al intentar crear el thread: %s",strerror(err));
		}
