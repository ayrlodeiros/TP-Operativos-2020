...
void iniciar_logger(void) {
	logger = log_create("server.log", "Modulo2", true, LOG_LEVEL_INFO);
}
...
void process_request(int cod_op, int cliente_fd) {
	...
	void* msg;
		switch (cod_op) {
		case MENSAJE:
			log_info(logger, "Se comenzo a procesar el mensaje del cliente");
...
void* recibir_mensaje(int socket_cliente, int* size)
{
	...
	log_info(logger, string_from_format("Se recibio el mensaje: %s", (char*) buffer));
